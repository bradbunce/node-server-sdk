...
//
// Returns an object with `port`, `hostname`, `requestCount()`, and `close()` so it can be used
// with the same `withCloseable` pattern as the HTTP test servers.
function startSocksProxyServer(options) {
  const opts = options || {};
...
module.exports = { startSocksProxyServer };
